...
if (references.length) {
  throw new Error(
    "krea2-local has no reference-image node in its ComfyUI graph, so it cannot honor " +
...
const scheduler = flags.scheduler ?? gen.scheduler ?? "simple";
...
const audit = await createImageGenerationAudit({
  ...
  provider: "krea2-local",
  service: {
    type: "ComfyUI Krea 2 Turbo",
    baseUrl,
    reachable: true,
...
// (type "krea2") + VAELoader, CLIPTextEncode into a zeroed-out negative like FLUX.2's distilled
// path, a plain EmptyLatentImage/KSampler pair (8 steps, cfg 1, euler/simple), then VAEDecode.
...
async function generate(specification) {
  const response = await fetch(`${baseUrl}/prompt`, {
    ...
    body: JSON.stringify({
      ...
      client_id: "youtube-pipeline-krea2",
    }),
  });
  if (!response.ok) throw new Error(`ComfyUI rejected Krea 2: ${await response.text()}`);
...
try {
  ...
  await audit.finish("completed", {
    ...
    usage: {
      ...
      estimatedCost: 0,
      quotaRemaining: null,
      quotaNote: "Local ComfyUI Krea 2 generation does not consume cloud credits.",
    },
  });
  console.log(`Krea 2 Turbo local finished in ${((Date.now() - startedAt) / 1000).toFixed(0)}s.`);
} catch (error) {
