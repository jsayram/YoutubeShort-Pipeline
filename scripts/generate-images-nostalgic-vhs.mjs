...
// Krea 2 Turbo, same graph as krea2-local, plus a LoraLoader chain for style LoRAs (e.g. the
// "2000s Analog Core" VHS/camcorder LoRA). Kept as its own provider/script so the plain
// krea2-local path stays untouched — this is an additive sibling, not a replacement.
...
if (references.length) {
  throw new Error(
    "nostalgic-vhs-2000 has no reference-image node in its ComfyUI graph, so it cannot honor " +
...
async function generate(specification) {
  const response = await fetch(`${baseUrl}/prompt`, {
    ...
    body: JSON.stringify({
      ...
      client_id: "youtube-pipeline-nostalgic-vhs",
    }),
  });
  if (!response.ok) throw new Error(`ComfyUI rejected nostalgic-vhs-2000: ${await response.text()}`);
...
try {
  ...
  console.log(`Nostalgic VHS 2000 finished in ${((Date.now() - startedAt) / 1000).toFixed(0)}s.`);
} catch (error) {
