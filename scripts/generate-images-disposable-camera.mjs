...
// Krea 2 Turbo, same graph as krea2-local, plus a LoraLoader chain for the Disposable Camera
// LoRA (cheap 35mm point-and-shoot film-scan look). Its own provider/script so krea2-local,
// nostalgic-vhs-2000, and graingaze-portrait stay untouched — additive sibling, not a replacement.
...
if (references.length) {
  throw new Error(
    "disposable-camera has no reference-image node in its ComfyUI graph, so it cannot honor " +
...
async function generate(specification) {
  const response = await fetch(`${baseUrl}/prompt`, {
    ...
    body: JSON.stringify({
      ...
      client_id: "youtube-pipeline-disposable-camera",
    }),
  });
  if (!response.ok) throw new Error(`ComfyUI rejected disposable-camera: ${await response.text()}`);
...
try {
  ...
  console.log(`Disposable camera finished in ${((Date.now() - startedAt) / 1000).toFixed(0)}s.`);
} catch (error) {
